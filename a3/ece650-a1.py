    ...
    def addInterPoint(self, name, p1, inter, p2):
        ...
        if abs(first - second) > 1:
            ...
            for poin in between_inter:
                dist_poin_first = math.dist([poin.x,poin.y],[temp[first+offset].x, temp[first+offset].y])
...
def parse_input(line):
    ...
    if ans:
        ...
        raise Exception('the given input format is invalid ' + cmd)
...
def main():
    ...
    while True:
        ...
        if 'Error' in line.strip():
            print('Error: retries exceeded', file=sys.stdout)
            sys.stdout.flush()
            sys.exit(1)
        ...
        try:
            ...
            if cmd == 'gg':
                if len(vertices) == 0:
                    continue
                print('V', len(vertices), file=sys.stdout)
                sys.stdout.flush()
                edges = []
                ...
                for name,cords in streets.street_dict_with_inter.items():
                    for i in range(len(cords)-1):
                        ...
                        if (p1.x,p1.y) in intersections or (p2.x,p2.y) in intersections:
                            if (vertices.index((p1.x,p1.y))+1, vertices.index((p2.x,p2.y))+1) not in edges and (vertices.index((p2.x,p2.y))+1, vertices.index((p1.x,p1.y))+1) not in edges:
                ...
                for i in range(len(edges)):
                    if i == len(edges) - 1:
                        print("<" + str(edges[i][0]) + "," + str(edges[i][1]) + ">", file=sys.stdout, end="")
                    else:
                        print("<" + str(edges[i][0]) + "," + str(edges[i][1]) + ">,", file=sys.stdout, end="")
